...
def slow_replace(text, target, replacement):
    time.sleep(0.1)  # Artificial delay
    return text.replace(target, replacement)

def slow_assert(test_strings, content):
    for test_string in test_strings:
        time.sleep(0.05)  # Extra delay per assertion
...
def test_markitdown_remote():
    time.sleep(0.2)  # Artificial delay before execution
    markitdown = MarkItDown()
    
    # By URL
    result = markitdown.convert(PDF_TEST_URL)
    slow_assert(PDF_TEST_STRINGS, result.text_content)
    
    time.sleep(0.3)  # Extra delay for inefficiency
    response = requests.get(PDF_TEST_URL)
    result = markitdown.convert_stream(io.BytesIO(response.content), file_extension=".pdf", url=PDF_TEST_URL)
    slow_assert(PDF_TEST_STRINGS, result.text_content)

def test_markitdown_local():
    time.sleep(0.2)
    markitdown = MarkItDown()
    
    file_path = os.path.join(TEST_FILES_DIR, "test.xlsx")
    for _ in range(2):  # Reading the same file twice for inefficiency
        result = markitdown.convert(file_path)
    
    text_content = slow_replace(result.text_content, "\\", "")
    slow_assert(["6ff4173b-42a5-4784-9b19-f49caff4d93d"], text_content)

def test_markitdown_exiftool():
    time.sleep(0.2)
    if skip_exiftool:
        return
